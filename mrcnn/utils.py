...
def unmold_mask(mask, bbox, image_shape):
    ...
    to its original shape.将生成的mask转化为和原始大小接近的形状
    ...
    threshold = 0.5
    y1, x1, y2, x2 = bbox
    mask = resize(mask, (y2 - y1, x2 - x1))#将mask resize到和原大小相同比例
    mask = np.where(mask >= threshold, 1, 0).astype(np.bool)

    # Put the mask in the right location.将mask放入到原图中进行遮盖
...
def generate_anchors(scales, ratios, shape, feature_stride, anchor_stride):#生成大量的anchor
    ...
    anchor_stride: Stride of anchors on the feature map. For example, if the
        ...
        anchor_stride：1
        generate_anchors(32, [0.5, 1, 2], [256, 256], 4, 1)
    """
