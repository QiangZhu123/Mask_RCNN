...
class MaskRCNN():#主函数
    ...
    def build(self, mode, config):#主方法
        ...
        h, w = config.IMAGE_SHAPE[:2]#图片大小
        if h / 2**6 != int(h / 2**6) or w / 2**6 != int(w / 2**6):#要求其是32的倍数
	...
	#RPN部分的GT输入
            ...
            #对RPN的结果检测的GT输入，分为三个部分，类，box，mask
        ...
        if callable(config.BACKBONE):#整个FPN网络的前半部分
